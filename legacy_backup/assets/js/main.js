...
function setupFormValidation() {
    ...
    if (messageField && charCounter) {
        const maxLength = parseInt(messageField.getAttribute('maxlength'), 10);
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            if (generalErrorContainer) {
                generalErrorContainer.textContent = 'Error: ' + error.message + ' Por favor, inténtalo de nuevo más tarde.';
            }
...
let currentNavIndex = 0; // Variable para rastrear el índice de navegación actual
...
function setupDirectPrint() {
    ...
    printCvLink.addEventListener('click', (e) => {
        e.preventDefault(); // Prevenir la navegación normal al hacer clic

        let iframe = document.getElementById('print-iframe-cv');
        if (!iframe) {
...
async function loadComponent(selector, url) {
    try {
        ...
        // Asegurarse de que los scripts que dependen de los componentes se ejecuten
        // incluso si uno de ellos falla, para no romper la funcionalidad básica.
 ...
 */
function updateMetaTag(selector, attribute, value, isLink = false) {
    ...
    if (!element) { // Si no existe, la crea
        ...
        selector.match(/\[(.*?)\]/g).forEach(attr => { const [key, val] = attr.slice(1, -1).split('='); element.setAttribute(key, val.replace(/"/g, '')); });
...
const getPageId = (path) => (path === '/' || path === '/index.html' || path === '') ? 'sobre-mi' : path.substring(1).replace(/\.html$/, '');
 ...
 * @param {HTMLElement} container - El contenedor del contenido principal.
 */
function renderFromCache(pageId, container) {
 ...
 * @param {HTMLElement} container - El contenedor del contenido principal.
 */
async function fetchAndRenderPage(pageId, container) {
 ...
 */
function showSkeleton(pageId, container) {
    const skeletonMap = {
 ...
 */
function parsePageFragment(html, pageId) {
...
function getNavIndexForPath(path) {
    const link = document.querySelector(`.nav-link[href="${path}"], .nav-link-mobile[href="${path}"]`);
    if (link && link.dataset.navIndex) {
 ...
 */
function announcePageLoad(title) {
 ...
 */
function updateAllMetaTags({ title, description, canonical, og }) {
 ...
 */
function postNavigationTasks(path, pageId, mainContent) {
    window.scrollTo(0, 0);
    setActiveNavLink();
 ...
 */
function initializePageScripts(pageId, contentContainer) {
